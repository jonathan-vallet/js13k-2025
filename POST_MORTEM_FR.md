...
Zelda _Oracle of Seasons_ a été une énorme source d’inspiration. J’ai repris le principe des saisons bloquant/débloquant des chemins, avec mes propres variantes :
...
Le printemps est la dernière saison débloquée, donc uniquement pensé comme _débloqueur_.  
...
Pour la carte, j’ai recyclé mon éditeur de niveaux en adaptant l’export : un gros JSON avec une alternance _cases vides / cases pleines_.  
...
Le gameplay 2024 était basé sur des déplacements _case par case_. Pour un open world, il fallait un **déplacement libre** et revoir entièrement collisions et mouvement.
...
- Gros travail d’équilibrage pour éviter que le joueur puisse se faufiler _entre deux cases_ ou coincer la hitbox dans un angle.

En rejouant à _Oracle of Seasons_, j’ai repéré un système malin : si 50% de la hitbox est déjà passée et qu’il n’y a plus d’obstacle, le personnage glisse automatiquement sur le côté.  
...
La zone sud, vide au départ, a été utilisée comme contenu _endgame_ → un chat nécessitant tous les pouvoirs des saisons.
...
J’ai utilisé [Soundbox](https://sb.bitsnbites.eu/) pour composer une musique simple, fortement inspirée de musiques de "Zelda: Oracle of Seasons". J'ai fait moins de SFX que l'année dernière, faute de place essentiellement. J'aurais aimé avoir un son pour localiser les chats en jouant sur la balance, ou quand on les récupère. Pareil pour les boules de feu.

En bidouillant les sons, j'ai changé un paramètre sur la longueur d'un son, et ça m'a permis de très simplement changer ce paramètre suivant les saisons. Printemps et été ont un son plus sec, tandis qu'automne et hiver ont un son plus résonnant.
...
Parfois, un _helper_ pour remplacer quelques appels ne vaut pas le coup : il est souvent plus efficace de répéter certains bouts de code, qui seront mieux compressés par Roadroller et le zip.
...
Le jeu a été très bien accueilli, positionnant Witchcat à la 3ème place du classement général de la game jam du [JS13K](https://js13kgames.com/). Beaucoup ont été impressionnés par la taille du monde et surtout le nombre d'assets différents. J'étais parti de ce défi technique, et j'en suis très content.

Tout le monde a plébiscité la direction artistique, les couleurs et l'ambiance générale. Le travail de Lylouf a été très apprécié!

Le changement de son suivant les saisons a été très apprécié, et pourtant ce n'est qu'un détail en terme de code, que j'ai trouvé en bidouillant les sons. C'est vrai qu'à titre personnel, j'adore les petits détails qui améliorent l'immersion, comme quand une musique change légèrement quand le personnage est sous l'eau avec un son plus étouffé.

Le level design a été très apprécié également. Grâce aux playtests en amont j'avais pu l'améliorer et c'était ma crainte principale. Ce n'est pas évident de doser des puzzles à la fois accessibles aux joueurs occasionnels, mais qui restent intéressants pour les joueurs plus expérimentés.

L'idée des panneaux pour indiquer les actions me paraissait bonne pour que ce soit diégétique, mais j'ai eu des retours mitigés. Certains ne savaient pas qu'on pouvait tuer les ennemis avec les boules de feu, bien que j'ai mis un panneau au début du jeu et une momie qui ne peut pas nous blesser pour "s'entrainer" après avoir détruit les premières fleurs.
J'aurais peut-être pu forcer la momie sur le passage, pour être sûr que le joueur comprenne le principe.

De même certaines personnes pensaient qu'il n'y avait pas de moyen de localiser les chats, alors qu'il y avait les panneaux disposés en cercle et remplacés par les chats collectés. Peut-être que le design et le fait que les panneaux soient interactif n'était pas évident. J'aurais pu essayere d'y mettre une bordure complètement noire pour les mettre un peu plus en évendence et qu'ils aient la même bordure que les fleurs qui peuvent être détruites (donc interactives), pratique qu'on retrouvait déjà dans les Zelda de gameboy.

Seule une personne a fait état de difficulté à se repérer et a demandé une mini-map. La carte n'était pas très grande et assez linéaire, je pensais que ça irait, mais je comprends que certains joueurs aiment avoir une mini-map pour se repérer. Le point bloquant peut être la sortie du temple de l'automne, où on peut retourner en arrière, notamment pour trouver un chat. Alors que la suite de l'aventure est sur la gauche en traversant des trous bouchés par les feuilles. J'avais mis des panneaux pour indiquer la direction des temples, mais une fois de plus très peu utilisés je pense.

2 retours ont été fait quant aux dalles pour changer de saison. Ce point a été beaucoup amélioré déjà visuellement et aussi grâce au fait que ce soit animé pour le rendre intuitif. Les joueurs vont dessus et testent spontanément le bouton action. Un panneau juste à côté indique aussi qu'on peut changer de saison, mais une fois de plus, les panneaux n'ont pas été très utilisés. En terme de level design, j'aurais pu faire une sorte de cul-de-sac avec la dalle au milieu, pour forcer le joueur à l'utiliser. Mais ça aurait bloqué un peu le world design en monde ouvert.
